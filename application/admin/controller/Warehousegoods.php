...
class WarehouseGoods extends Common
{
    ...
    public function Index()
    {
        $admin = $this->admin;
        if($admin['role_id'] != 1){
            $where = ["warehouse_id" , "in" ,[$admin['warehouse_id']]];
        ...
        if($admin['role_id'] != 1){
            $data_params['where']['id'] = $admin['warehouse_id'];
        }
    ...
    public function InventoryInfo()
    {
        ...
        $admin = $this->admin;
        $param['where'] = ['id' => $admin['warehouse_id']];
        $warehouse = WarehouseService::WarehouseList($param);
        ...
        // 数据
        $this->assign('city_name',$warehouse['data'][0]['city_name']);
        $this->assign('data', $data);
