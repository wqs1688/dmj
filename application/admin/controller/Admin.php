...
class Admin extends Common
{
	...
	public function SaveInfo()
	{
        ...
        $area_list = WarehouseService::getWarehouses();
    ...
    public function setLang(){
        ...
        $p = $this->data_post;
        $lang_type = $p["lang_type"];
        $admin_id  = $p["admin_id"];
        if(Db::name("Admin")->where(["id"=>$admin_id])->update(["lang_type"=>$lang_type])){
            $admin = Db::name('Admin')->field('id,username,lang_type,mobile,email,login_total,role_id,warehouse_id')->where(['id'=>$admin_id, 'status'=>0])->find();
            session('admin_login', $admin);
            return DataReturn('设定成功', 0);
        }
        return DataReturn('設定失敗', -100);
    }
}
?>
