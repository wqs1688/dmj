...
class Goods extends Common
{
    ...
    public function Detail()
    {
        if(!empty($this->data_request['id']))
        {
            ...
            $where = [
                ['g.is_delete_time', '=', 0],
                ['g.id', '=', intval($this->data_request['id'])],
            ];
	...
	public function SaveInfo()
	{
        ...
        $express = 17;
        $expressPrice = '';
        $expressPriceList = ExpressService::getExpressPrice($express);
		if(!empty($params['id']))
		{
			// 条件
            $where = [
                ['g.is_delete_time', '=', 0],
                ['g.id', '=', intval($params['id'])],
            ];
            ...
            if($expressPrice){
                $expressList = explode(" ",$expressPrice);
                $express = $expressList[0];
                $expressPriceList = ExpressService::getExpressPrice($express,2);
            }
		}
        $this->assign('expressPrice',$expressPrice);
        $this->assign('express',$express);
        $this->assign('expressPriceList',$expressPriceList);
        $params['is_enable'] = 1;
        $expresslist = ExpressService::ExpressList($params);
        ...
        $this->assign('role_id',$this->admin['role_id']);
		return $this->fetch();
	}
    ...
    public function GetPrice()
    {
        ...
        return ExpressService::getExpressPrice($express_id);
    }
}
?>
