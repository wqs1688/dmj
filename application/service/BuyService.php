...
class BuyService
{
    ...
    public static function GoodsSpecificationsHandle($params = [])
    {
        ...
        if(!empty($params['spec']))
        {
            ...
            {
                $spec = htmlspecialchars_decode($params['spec']);
                $spec = json_decode(htmlspecialchars_decode($spec), true);
                if(!is_array($spec)){
    ...
    public static function CartList($params = [])
    {
        ...
        $field = 'c.*, g.inventory_unit, g.is_shelves, g.is_delete_time, g.buy_min_number, g.buy_max_number, g.model, g.site_type, g.express_price';
    ...
    public static function BuyGoods($params = [])
    {
        ...
        $p = [
            ...
            'field' => 'g.id, g.id AS goods_id, title, images, inventory_unit, buy_min_number, buy_max_number, model, express_price',
        ];
     ...
     * @date    2022-01-09
     */
    public static function AddrFileUpload($params = [])
    {
        // 参数校验
        if(! isset($params['id']))
        {
            return DataReturn('缺少参数', -4);
        }
        ...
        // 文件格式化校验
        $type = ResourcesService::PdfExtTypeList();
        if(!in_array($_FILES['file']['type'], $type))
        {
            return DataReturn('文件格式有误，请上传 PDF 文件', -2);
        }

        // 文件大小校验
        if($_FILES['file']['size'] > 2048000)
        {
            return DataReturn('文件太大，请上传 2M 以内文件', -3);
        }

        $uploader = '\base\\Uploader';
        $config = [
            'pathFormat' => __MY_ROOT_PUBLIC__ . 'static/upload/file/addrpdf/{yyyy}-{mm}-{dd}-' . $params['id'],
            'maxSize' => 2048000,
            'allowFiles' => ['.pdf'],
        ];
        // 文件上传
        $obj = new $uploader('file', $config, 'file');

        // 判断文件上传成功
        $d = explode('-', date("Y-m-d"));
        $format = trim($config['pathFormat'], '/');
    ...
    public static function OrderInsert($params = [])
    {
        ...
        foreach($buy['data']['goods'] as $v)
        {
            ...
            if(in_array($site_model, [0,2]))
            {
                if(isset($params['address_id']) && empty($v['order_base']['address']))
                {
            ...
            $order = [
                ...
                'out_fee'               => ($v['order_base']['out_price'] <= 0.00) ? 0.00 : $v['order_base']['out_price'],
                'handing_fee'           => ($v['order_base']['use_price'] <= 0.00) ? 0.00 : $v['order_base']['use_price'],
                'express_fee'           => ($v['order_base']['express_price'] <= 0.00) ? 0.00 : $v['order_base']['express_price'],
    ...
    private static function OrderDetailInsert($order_id, $user_id, $detail)
    {
        $data = [
            ...
            'express_fee'       => $detail['express_price'],
    ...
    private static function OrderAddressInsert($order_id, $user_id, $address)
    {
        ...
        // 上传地址文件
        $addr_file = '';
        if(isset($address['type']) && $address['type'] == 'file')
        {
            $upd_info = [
                'id' => $order_id,
            ];
            $addr_file = self::AddrFileUpload($upd_info);
        }
                ...
                if(empty($temp))
                {
                    ...
                    if(isset($goods['is_deduction_inventory']) && $goods['is_deduction_inventory'] == 1)
                    {
                        ...
                        if($base['code'] == 0)
                        {
                            ...
                            $sku = $base['data']['spec_base']['goods_sku'] ?? '';

                            // 仓库库存扣除
                            $we_ret = WarehouseGoodsService::WarehouseGoodsInventoryDeduct($params['order_id'], $v['goods_id'], $spec, $v['buy_number'], $sku);
                            if($we_ret['code'] != 0)
                            {
                                return $we_ret;
                            }
