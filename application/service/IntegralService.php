...
class IntegralService
{
     ...
     */
    public static function UserIntegralLogAdd($user_id, $original_integral, $operation_integral, $msg = '', $type = 0, $operation_id = 0, $order_num = '')
    {
        $data = array(
            'user_id'               => intval($user_id),
            'original_integral'     => $original_integral,
            'operation_integral'    => $operation_integral,
            'order_num'             => $order_num,
            'msg'                   => $msg,
            'type'                  => intval($type),
    ...
    public static function IntegralLogList($params = [])
    {
        ...
        if(!empty($data))
        {
            ...
            foreach($data as &$v)
            {
                ...
                $type = $v['type'] == 1 ? '+' : '-';
                // 操作积分
                $v['operation_integral'] = $type . $v['operation_integral'];

                // 订单号
                $v['order_num'] = $v['order_num'] != '' ? $v['order_num'] : '--';
