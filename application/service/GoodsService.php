...
class GoodsService
{
    ...
    public static function HomeFloorList($params = [])
    {
                    ...
                    switch($floor_data_type)
                    {
                        ...
                        case 0 :
                            if(isset($goods_count) && isset($order_by))
                            {
                                ...
                                $v['goods_ids'] = Db::name('Goods')->alias('g')->join(['__GOODS_CATEGORY_JOIN__'=>'gci'], 'g.id=gci.goods_id')->join(['__WAREHOUSE_GOODS__'=>'wg'], 'g.id=wg.goods_id and wg.inventory>0')->where($where)->group('g.id')->order($order_by)->limit($goods_count)->column('g.id');
                            }
                            break;
        ...
        if(!empty($data) && is_array($data))
        {
            ...
            {
                $where = [
                    ['g.id', 'in', array_unique($goods_ids)],
                    ['is_shelves', '=', 1],
                ];
                $res = self::GoodsList(['where'=>$where, 'm'=>0, 'n'=>0, 'field'=>'g.*']);
    ...
    public static function GoodsDataHandle($data, $params = [])
    {
        if(!empty($data))
        {
            ...
            foreach($data as &$v)
            {
                ...
                // 商品公开数量
                if(isset($v['inventory']) && isset($v['inventory_private']))
                {
                    $v['inventory_public'] = $v['inventory'] - $v['inventory_private'];
                }
                ...
                if(!empty($v['add_time']))
                {
                    $v['add_time'] = date('Y-m-d H:i:s', intval($v['add_time']));
                }
                if(!empty($v['upd_time']))
                {
                    $v['upd_time'] = date('Y-m-d H:i:s', intval($v['upd_time']));
                }
    ...
    {
        return (int) Db::name('Goods')->alias('g')->join(['__GOODS_PUBLISH__'=>'p'],'g.id=p.goods_id')->join(['__USER__'=>'u'],'u.id=p.user_id')->where($where)->count();
    }
    ...
    public static function GoodsList($params = [])
    {
        ...
        $data = Db::name('Goods')->alias('g')->join(['__GOODS_PUBLISH__'=>'p'],'g.id=p.goods_id')->join(['__USER__'=>'u'],'u.id=p.user_id')->field($field)->where($where)->order($order_by)->limit($m, $n)->select();
    ...
    public static function GoodsSave($params = [])
    {
        ...
        try {
            ...
            {
                $data['add_time'] = time();
                $data['express_price'] = $params["expressPrice"];
        ...
        if(!empty($params['spec']))
        {
            ...
            if(!empty($ids))
            {
                ...
                if(!empty($temp_data))
                {
                    ...
                    if(!empty($base_id))
                    {
                        $base = Db::name('GoodsSpecBase')->alias('gsb')->join(['__WAREHOUSE_GOODS__'=>'wg'],'gsb.barcode=wg.goods_sku','left')->find($base_id);
                    }
                }
            }
        } else {
            $base = Db::name('GoodsSpecBase')->alias('gsb')->join(['__WAREHOUSE_GOODS__'=>'wg'],'gsb.barcode=wg.goods_sku','left')->where('wg.goods_id', intval($params['id']))->find();
        }
     ...
     */
    // public static function FakeInventory($id)
    // {
    //     $data_params = [
    //         'm'             => 0,
    //         'n'             => 99999,
    //         'where'         => [
    //             ['w.goods_id', '=', intval($id)],
    //             ['w.status', '<>', 1],
    //         ],
    //         'field'         => 'w.*',
    //     ];
    //     $ret = UserWarehouseService::UserWarehouseList($data_params);
    //     $goods_warehouse = (empty($ret['data']) || empty($ret['data'][0])) ? [] : $ret['data'];

    //     // 未实际入库的数量
    //     $fake_inventory = 0;
    //     foreach($goods_warehouse as $v)
    //     {
    //         $fake_inventory += $v['goods_num'];
    //     }
