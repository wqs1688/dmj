...
class AdminService
{
    ...
    public static function AdminList($params = [])
    {
        ...
        if(!empty($data))
        {
            ...
            foreach($data as &$v)
            {
                ...
                $ret = Db::name('Warehouse')->where(['id'=>$v['warehouse_id']])->field('name')->select();
                $v['area_name'] = $ret[0]['name'];
                // 时间
    ...
    public static function Login($params = [])
    {
        ...
        // 获取管理员
        $admin = Db::name('Admin')->field('id,username,lang_type,mobile,email,login_pwd,login_salt,login_total,role_id,warehouse_id')->where([$ac['data']=>$params['accounts'], 'status'=>0])->find();
        if(empty($admin))
        {
    ...
    public static function getLanguesList(){
        $ret = Db::name('Langue')->where(["del_flg"=>0])->field('id,langue')->select();
        return $ret;
    }
}
?>
