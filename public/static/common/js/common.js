...
function FormDataFill(json, tag)
{
	if(json != undefined)
	{
		...
		for(var i in json)
		{
            ...
            if(i == "method"){
                var method_html = '<tr><td class="am-text-middle"><input type="text" placeholder="配送方法" name="method[]" minlength="2" maxlength="16" data-validation-message="名称格式 2~16 个字符" class="am-radius" required /></td><td class="am-text-middle"><input type="number" name="price[]" placeholder="运费" value="" data-validation-message="请填运费金额" maxlength="6" required /></td><td class="am-text-middle"><span class="am-text-xs cr-blue c-p am-margin-right-sm line-move" data-type="top">上移</span><span class="am-text-xs cr-blue c-p am-margin-right-sm line-move" data-type="bottom">下移</span><span class="am-text-xs cr-red c-p line-remove">移除</span></td></td></tr>';;
                $form.find('.parameters-table tbody').children().remove();
                if(json[i].length == 0){
                    $form.find('.parameters-table').append(method_html);
                }else{
                    for(var m in json[i]){
                        method_html = '<tr><td class="am-text-middle"><input type="text" placeholder="配送方法" name="method[]" value="'+json[i][m]["method_name"]+'" minlength="2" maxlength="16" data-validation-message="名称格式 2~16 个字符" class="am-radius" required /></td><td class="am-text-middle"><input type="number" name="price[]" placeholder="运费" value="'+json[i][m]["method_price"]+'" data-validation-message="请填运费金额" maxlength="6" required /></td><td class="am-text-middle"><span class="am-text-xs cr-blue c-p am-margin-right-sm line-move" data-type="top">上移</span><span class="am-text-xs cr-blue c-p am-margin-right-sm line-move" data-type="bottom">下移</span><span class="am-text-xs cr-red c-p line-remove">移除</span></td></td></tr>';
                        $form.find('.parameters-table').append(method_html);
                    }
                }
            }
...
function ConfirmDataDelete(e)
{
    ...
    var text1 = e.attr('data-cancel') || '取消';
	var text2 = e.attr('data-sure') || '确认';
...
$(function()
{
    ...
    $('.form-table-operate-top-delete-submit').on('click', function()
    {
        ...
        var text1 = $(this).attr('data-cancel') || '取消';
        var text2 = $(this).attr('data-sure') || '确认';
	...
	// 弹出新增页面
    // 表格公共新增操作
    $('.form-table-operate-top-add-submit').on('click', function()
    {
        ModalLoad($(this).data('url'), $(this).data('popup-title'), 'common-add-modal');
	...
	{
		// 省初始化
        var first = $('.region-linkage select[name=province]').attr('data-first') || 0;
		RegionNodeData(first, 'province', 'province');
    ...
    // 根据邮编检索地址
    $('input[name=zips]').on('blur', function(){
        var zip = $('input[name=zips]').val();
        ...
        $.ajax({
            ...
            success:function(result)
            {
                if(result.code == 0)
                {
                    ...
                    $('.region-linkage select[name=county]').attr('data-value', result.data.country);

                     var first = $('.region-linkage select[name=province]').attr('data-first') || 0;
                     RegionNodeData(first, 'province', 'province');

                     if(result.data.province != 0)
                     {
                        RegionNodeData(result.data.province, 'city', 'city');
                     }

                     if(result.data.city != 0)
                     {
                         RegionNodeData(result.data.city, 'county', 'county');
                     }
                } else {
                    Prompt(result.msg);
                }
