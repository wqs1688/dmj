<view qq:if="{{detail != null}}">
  <form bindsubmit="formSubmit" class="form-container">
      <view qq:for="{{detail.items}}" qq:key="item" class="form-gorup bg-white oh">
        <view class="oh">
          ...
          <view class="item-base">
            <block qq:for="{{[1,2,3,4,5]}}" qq:key="tx" qq:for-index="tx">
            ...
            <text qq:if="{{form_rating_list[index] != undefined}}" class="msg-text cr-888">{{rating_msg[form_rating_list[index]-1]}}</text>
          </view>
        </view>
        <view class="form-content br-t">
          ...
          <view class="form-container-upload oh">
            <view class="form-upload-data fl">
              <block qq:if="{{(form_images_list[index] || null) != null && form_images_list[index].length > 0}}">
                <view qq:for="{{form_images_list[index]}}" qq:key="item" qq:for-index="ix"  class="item fl">
            ...
            <image qq:if="{{(form_images_list[index] || null) == null || form_images_list[index].length < 3}}" class="upload-icon" src="/images/default-upload-icon.png" mode="aspectFill" bindtap="file_upload_event" data-index="{{index}}" />
          </view>
        </view>
      </view>
